...
(function(){
  ...
  function ensureLayer(){
    ...
    if (!el){
      ...
      el.innerHTML = `<img style="max-width:${PREVIEW_W}px;border-radius:6px;box-shadow:0 0 8px rgba(0,0,0,.5);" />`;
  ...
  function showAt(x, y, cd){
    ...
    let left = (Number(x) || 0) + PREVIEW_PAD;
    let top  = (Number(y) || 0) + PREVIEW_PAD;

    if (left + PREVIEW_W > vw) left = Math.max(PREVIEW_PAD, (Number(x)||0) - PREVIEW_W - PREVIEW_PAD);
...
window.hideCardPreview   = window.hideCardPreview   || (()=>window.CardPreview.hide());
  ...
  if (!window.__cardPreviewCloseWired){
    ...
    document.addEventListener('click', (e) => {
      ...
      if (pop && pop.style.display !== 'none' && !e.target.closest('#card-preview-pop')) hide();
    });
  }
})();